const Game = (function() {
    ...
    let fogEnabled = true; // <-- NEW: Track fog visibility
    ...
    const setupInput = function() {
      window.addEventListener('keydown', (e) => {
        ...
        // --- BEGIN: Spacebar fog toggle ---
        if (e.code === 'Space' && fogEnabled) {
          fogEnabled = false;
        }
        // --- END: Spacebar fog toggle ---
      });
      ...
      showEndingScreen: (result, time, records) => {
        ...
        if (records && records.length > 0) {
          list.innerHTML = records.map((rec, i) => `
            <li>${i + 1}. ${rec.name || 'Player'} - ${rec.time}s</li>
          `).join('');
        } else {
